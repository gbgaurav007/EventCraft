...
function EventCarousel({ events, title, eventsToShow = 3, handleViewDetails, handleAddEvent, userData }) {
        ...
        <div className="mt-10 px-5 mb-10">
            <div className="flex justify-between items-center mb-5">
                ...
                <div className='flex flex-row gap-5'>
                    {userData?.role === 'Admin' || userData?.role === 'Event Organizer' ? (
                        <button onClick={handleAddEvent} className="text-blue-500 font-semibold">
                            Add event
                        </button>
                    ) : null}
                ...
                >
                    {events.map((event, index) => (
                        <div key={index} className="flex-none md:w-1/3 w-full p-2">
                            <EventCard event={event} handleViewDetails={handleViewDetails} />
                        </div>
                    ))}
                </motion.div>
